...
if(isset($_POST['registrar']) && !empty($_POST['nombre']) && !empty($_POST['apellido']) && !empty($_POST['contrasena']) && !empty($_POST['tipo_documento']) && !empty($_POST['documento']) && !empty($_POST['estado_civil']) && !empty($_POST['programa'])){
    ...
    $consultadoc =$mysql->efectuarConsulta("SELECT numero_documento FROM id11714256_biblioteca3.bibliotecario WHERE numero_documento = ".$ndocumento."");

    //declaracion de la variable donde se almacena la funcion de mysql.php donde se hara la respectiva consulta
    $consulta =$mysql->efectuarConsulta("SELECT numero_documento FROM id11714256_biblioteca3.estudiantes WHERE numero_documento = ".$ndocumento."");
    ...
    if(mysqli_num_rows($consultadoc)){
       ...
       if(mysqli_num_rows($consulta)>0){
        ...
        header("refresh:1;url=../register_usuario.php");
            ...
            $sql=$mysql->efectuarConsulta("insert into id11714256_biblioteca3.estudiantes(numero_documento,nombre,apellido,contrasena,tipo_documento_id_tipo,estado_civil_id_estado,programa_id_programa, estado) VALUES (".$ndocumento.",'".$nombre."','".$apellido."','".$pass."','".$documento."','".$estado."','".$programa."',1)");
            //mensaje de salida (alerta) junto a su redireccion de pagina
